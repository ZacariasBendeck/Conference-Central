...
# - - - Conference objects - - - - - - - - - - - - - - - - -
    ...
    def queryConferences(self, request):
        ...
        for organiser in organisers:
            logging.info(' oganisers!! : %s', organiser)
...
# - - - Registration - - - - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        query = Conference.query(Conference._properties['name'] == request.conferenceName).get()
        if not query:
            raise endpoints.NotFoundException(
                "No conference found with this name: %s", data['name'])
        conference = query
        logging.info('Found conference to add Session to = %s', conference.name)
        ...
        #Get conference Sessions endpoint
    @endpoints.method(CONF_GET_REQUEST, SessionForms,
            path='sessions/attending',
            http_method='GET', name='getConferenceSessions')
    def getConferenceSessions(self, request):
        """Get list of conferences that user has registered for."""
        #Get conference key and make sure it exists
        wsck = request.websafeConferenceKey
        conference = ndb.Key(urlsafe=wsck).get()
        if not conference:
            raise endpoints.NotFoundException(
                'No conference found with key: %s' % wsck)
        logging.info('found conference name : %s', conference.name)
        # get session keys
        session_keys = [(ndb.Key(urlsafe=wsck)) for wsck in conference.sessionKeys]
        session_keys2 = []
        for wsck in conference.sessionKeys:
            session_keys2.append(ndb.Key(urlsafe=wsck))
            logging.info('appended to session keys: %s', ndb.Key(urlsafe=wsck))
            logging.info('testing to append websafekeyinstead: %s', ndb.Key(urlsafe= wsck).urlsafe())

        logging.info('Found these session_keys : %s', session_keys)
        logging.info('The first key is : %s', session_keys[0])
        ...
        """names = {}
        for session in sessions:
            names[session.key.id()] = session.key()
            logging.info('session names : %s', session.key())
    ...
    def _copySessionToForm(self, session):
        ...
        for field in sf.all_fields():
            if hasattr(session, field.name):
                logging.info('Entering loop %s %s', field.name, getattr(session, field.name))
                # convert Date to date string; just copy others
                if field.name.endswith('Date'):
            ...
            #elif field.name == "websafeKey":
             #   logging.info(' is there a websafe key?? ')
